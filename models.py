...
class OllamaProvider:
    ...
    def chat(
        ...
        **kwargs
        ...
        # remove stream from ollama options (fixed typo)
        ollama_options.pop("stream", None)

        # Reduced context window to prevent OOM errors on consumer hardware
        # 2048 is enough for resume sections while keeping memory usage low
        ollama_options["num_ctx"] = 2048

        # Set temperature to 0 to prevent hallucination loops
        # Small models can get stuck repeating when temperature > 0
        ollama_options["temperature"] = 0

        # Set a reasonable token limit to prevent infinite loops
        ollama_options["num_predict"] = 1024

        # Set repeat_penalty to discourage repetition
        ollama_options["repeat_penalty"] = 1.1
...
class DeepSeekProvider:
    """DeepSeek API provider implementation (OpenAI-compatible endpoint)."""
    ...
    def chat(
        ...
        **kwargs
        ...
        # If the caller asked for structured JSON output (same "format" kwarg
        # used for the Ollama/Gemini schema calls), request DeepSeek's JSON mode.
        if "format" in kwargs:
        ...
        # Convert to the same Ollama-like shape the rest of the app expects
